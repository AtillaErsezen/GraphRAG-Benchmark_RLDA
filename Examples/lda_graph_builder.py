...
def perform_lda_on_layer(docs: List[str], depth: int, chunk_token_size: int = SUB_CHUNK_TOKEN_SIZE) -> Optional[Dict]:
    """
    Fit LDA on `docs` at the given recursion depth.

    depth=0: corpus chunks → up to GLOBAL_N_TOPICS topics.
             Each chunk is sub-chunked and recursed with chunk_token_size halved each level.
    depth≥1: sub-chunks → up to SUB_N_TOPICS topics; recursion stops when word_count <= 50.
    ...
    for i, content in enumerate(docs):
        ...
        # documents with <= 50 words cannot be meaningfully sub-chunked further
        word_count = len(content.split())
        if word_count <= 50:
            sub_tree = {"status": "leaf", "content": content}
        else:
            # halve the chunk size at each level so sub-chunks shrink and eventually fall below the 50-word leaf threshold
            next_chunk_size = chunk_token_size // 2
            raw = chunking_by_token_size(
                content,
                overlap_token_size=SUB_CHUNK_OVERLAP,
                max_token_size=next_chunk_size,
            )
            ...
            if len(sub_texts) <= 1:
                ...
                print(f"[LDA] depth={depth} | chunk[{i}] → {len(sub_texts)} sub-chunks (token_size={next_chunk_size})")
                sub_tree = perform_lda_on_layer(sub_texts, depth + 1, next_chunk_size)
